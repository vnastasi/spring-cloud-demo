...
import md.vnastasi.cloud.client.model.disruption.TravelAdviceWrapper
import md.vnastasi.cloud.client.model.notification.NotificationWrapper
import md.vnastasi.cloud.endpoint.model.*

fun TravelAdviceWrapper.asTravelAdvice(): TravelAdvice =
    TravelAdvice(summary = this.title, advice = this.advice.toList())

fun TravelAdviceListWrapper.asTravelAdviceHolder(): TravelAdviceHolder =
    TravelAdviceHolder(summary = this.title, adviceList = this.travelAdvices.map { it.asTravelAdvice() })
...
fun DisruptionWrapper.asDisruption(): Disruption =
    Disruption(
        ...
        travelAdvice = this.travelAdvices.asTravelAdviceHolder(),
